...
#install.packages('/gpfs/gibbs/pi/gerstein/en325/ENTEX2/AlleleSeq2/VGAM/VGAM_1.1-10.tar.gz', repos=NULL, lib='/gpfs/gibbs/pi/gerstein/en325/ENTEX2/AlleleSeq2/VGAM/vgam_lib')
#library(VGAM)
...
# x11(width=17, height=9)
pdf(file=paste(filename,"-checkgrad-",minN,"-",maxN,".pdf", sep=""),width=17, height=9)
par(cex.axis=1.5, cex.lab=2, cex.main=2, mar=c(5,5,5,5))
barplot(empirical, ylab='density', xlab='ref_allele_ratio', 
        names.arg=h$mids, ylim=c(0,yuplimit), main=paste("n=",minN,'-',maxN))
par(new=TRUE)
...
pdf(paste(file=filename,"-checksse.grad-",minN,"-",maxN,".pdf", sep=""), width=10, height=7)
par(cex.axis=1.5, cex.lab=2, cex.main=2, mar=c(5,5,5,5))
