...
VCF2DIPLOID_DIR               := /vast/palmer/pi/gerstein/mj648/HGSVC3_ASE/vcf2diploid
...
LIFTOVER                      := /vast/palmer/apps/avx2/software/liftOver/2023-05-23/liftOver
...
REFGENOME_VERSION             := GRCh37   #GRCh38 or GRCh37 or chm13 (T2T)
...
## Primary assembly
  ...
  ADDNL_SEQNS      := /gpfs/gibbs/pi/gerstein/from_slayman/tg397/ENTEx_Allele_share/JQL/hg38.analysisSet_unplaced_unlocalized_EBV_contigs_only.fasta

  ANNOTATION       := /vast/palmer/pi/gerstein/jl3786/Genomes/hg38/annotation/gencode.v47.annotation.gtf

else ifeq ($(REFGENOME_VERSION), chm13)

  REFGENOME        := /vast/palmer/pi/gerstein/mj648/HGSVC3_ASE/references/hs1.fa
  ADDNL_SEQNS      := 
  ANNOTATION       := /vast/palmer/pi/gerstein/mj648/HGSVC3_ASE/references/chm13v2.0_RefSeq_Liftoff_v5.1.gff3
...
$(OUTPUT_DIR)/maternal.chain $(OUTPUT_DIR)/paternal.chain: $(FILE_PATH_VCF)
	...
	-outDir $(OUTPUT_DIR) #Removed -pass parameter
	mv *.chain *$(VCF_SAMPLE_ID)*.fa *$(VCF_SAMPLE_ID)*.map $(OUTPUT_DIR) #new command
...
$(OUTPUT_DIR)/$(VCF_SAMPLE_ID)_hetSNVs_ref.bed: $(FILE_PATH_VCF) $(OUTPUT_DIR)/maternal.chain $(OUTPUT_DIR)/paternal.chain
	cat $(FILE_PATH_VCF) | python /vast/palmer/pi/gerstein/mj648/HGSVC3_ASE/AlleleSeq2/get_hetSNV_bed_new.py $(VCF_SAMPLE_ID) $(REFGENOME) > $@
...
$(OUTPUT_DIR)/$(VCF_SAMPLE_ID)_hetSNVs_rd.tab: $(OUTPUT_DIR)/$(VCF_SAMPLE_ID)_hetSNVs_ref.bed
	...
	$(BEDTOOLS_intersectBed) intersect -a $(FILE_PATH_BAM) -b stdin \
		-g $(OUTPUT_DIR)/genome.txt -wb -bed | python $(PL)/calculate_rd.py > $@
